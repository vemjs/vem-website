...
export class HomeView extends UIComponent {
  ...
  constructor(width: number, height: number, onLaunch: () => void, onViewDocs: () => void, onViewConfig: () => void) {
    super();
    this.width = width;
    this.height = height;
    ...
    this.secondaryBtn = new Button('GitHub Repository', {
      onClick: () => window.open('https://github.com/vemjs/vem', '_blank', 'noopener'),
      bg: '#1e293b', // slate-800
      hoverBg: '#334155',
      color: '#ffffff',
      font: '600 16px Outfit, sans-serif',
  ...
  private createFeatureCard(title: string, body: string, linkText: string, onClick: () => void): Card {
    const card = new Card({
      width: 400,
      height: 180,
      bg: '#111827', // grey-900
